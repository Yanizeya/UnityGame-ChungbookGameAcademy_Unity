...
public class DateCheck : MonoBehaviour
{
    ...
    private AudioSource univoice;   //다음팟 플레이어
    public AudioClip voiceBirthday; //mp4파일, 위의 AudioSource에 넣어서 플레이
    ...
    void Start()
    {
        ...
        //이전에 실행했던 날짜 얻기
        int oldMonth = PlayerPrefs.GetInt("Month"); //PlayerPrefs는 데이터 관리 클래스, 컴퓨터에 데이터 저장
        int oldDay = PlayerPrefs.GetInt("Day");
        Debug.Log("이전 실행일 : " + oldMonth + "월 " + oldDay + "일\n" + "현재 실행일 : " + nowMonth + "월 " + nowDay + "일");

        // 해당 날짜의 음성이 있으면 재생
        univoice = GetComponent<AudioSource>();
        if (voiceDate[nowMonth, nowDay] != null && (oldMonth != nowMonth || oldDay != nowDay)) //해당 날짜에 이미 재생을 했다면 반복 재생을 하지 않는다
            univoice.PlayOneShot(voiceDate[nowMonth, nowDay]);

        //현재 실행한 날짜 기록
        PlayerPrefs.SetInt("Month", nowMonth); //PlayerPrefs에다 현재 실행한 날짜 셋팅
